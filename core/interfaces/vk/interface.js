...
module.exports = class VKInterface extends Interface {
    ...
    startup(config) {
        ...
        var vk_config = config.retrieve("vk",
            {
                group_id: new ValueEntry("number", Utils.isValid(process.env.group_id) ? +process.env.group_id : null), //process.env converts anything into string so we need to convert is to integer again
